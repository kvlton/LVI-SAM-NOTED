...
class FeaturePerId
{
  public:
    ...
    int start_frame; // 起始frame
    vector<FeaturePerFrame> feature_per_frame; // 每个特征点由连续的几个frame观测到

    int used_num;
    bool is_outlier;
    bool is_margin;
    double estimated_depth; // 特征点的深度
    bool lidar_depth_flag;  // 是否使用liadr得到的深度
...
class FeatureManager
{
  public:
    ...
    list<FeaturePerId> feature; // 特征点集合
    int last_track_num; // 跟踪到的特征点数量
